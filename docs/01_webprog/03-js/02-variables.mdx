...
| Array   | A data structure able to store multiple values  | `let arr = [1, 'ABC', {}]` (note: we can use multiple data types in this structure)
| Boolean   | A binary value being either true or false (representing '1' or '0') | `let bool = true`

What stands out again in all these examples is the absence of a distinct type declaration that you would expect from strongly typed languages such as Java - in JavaScript, there 
is no need to explicitly state the variable's data type.